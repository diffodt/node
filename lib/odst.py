...
class ODST(ModuleWithInit):
    ...
    def forward(self, input):
        ...
        feature_selectors = sparsemax(feature_logits, dim=0)
    ...
    def initialize(self, input, eps=1e-6):
        ...
        with torch.no_grad():
            feature_selectors = sparsemax(self.feature_selection_logits, dim=0)
            # ^--[in_features, num_trees, depth]
            ...
            temperatures = np.percentile(check_numpy(abs(feature_values - self.feature_thresholds)),
                                         q=100 * self.threshold_init_cutoff, axis=0)
